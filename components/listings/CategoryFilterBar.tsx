      ...
      <div className="flex items-center gap-2 min-w-max">
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              key={cat.id}
              onClick={() => handleSelect(cat.id, (cat as any).type, (cat as any).isTrade)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-[20px] text-xs sm:text-sm font-bold transition-all duration-200 cursor-pointer ${
                isSelected
            ...
            >
              {Icon && <Icon className={`w-4 h-4 ${isSelected ? "text-primary-fixed" : "text-primary"}`} />}
