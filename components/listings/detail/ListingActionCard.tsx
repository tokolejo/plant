...
export function ListingActionCard({
  ...
  const safePhone = phone || "555 000 000";
  const maskedPhone = showFullPhone
    ? safePhone
    : safePhone.slice(0, 6) + " ***";

  const cleanWaNumber = (whatsapp || phone || "").replace(/[^0-9]/g, "");
  const waUrl = cleanWaNumber
    ? `https://wa.me/${cleanWaNumber}?text=${encodeURIComponent(
        isKa
          ? `გამარჯობა! დავინტერესდი თქვენი მცენარით: "${title}" (Plantio.ge-დან)`
          : `Hello! I'm interested in your plant: "${title}" from Plantio.ge`
      )}`
    : null;

  // Active delivery methods (filter out false/unavailable)
  ...
  if (deliveryOptions?.post) {
    activeDeliveryMethods.push({ label: isKa ? "საქართველოს ფოსტა" : "Post Delivery", icon: Truck });
  }
  ...
  return (
    <div className="rounded-[22px] border border-border/80 bg-card p-4 sm:p-5 shadow-xs space-y-4">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {/* Action icons: Wishlist, Share, Copy */}
      ...
      <div className="pt-3 pb-3 border-y border-border/60 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-800 dark:text-emerald-300 border border-emerald-500/20">
          ...
          <span>{isKa ? "დაცული გარიგება" : "Protected"}</span>
        </div>
      </div>

      {/* ── Delivery Methods (Clean, Only Available) ── */}
      {activeDeliveryMethods.length > 0 && (
        <div className="space-y-1.5">
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
            {isKa ? "მიწოდების მეთოდები:" : "Available Delivery:"}
          </span>
      ...
      {/* ── PRIMARY CTA: Escrow / Propose Swap / Claim ── */}
      <div className="pt-1 space-y-2.5">
        <Button
          type="button"
          onClick={onPrimaryAction}
          className={`w-full h-11 px-4 rounded-[14px] font-black text-sm shadow-xs flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-[0.99] ${
            transactionType === "TRADE"
        ...
        >
          {transactionType === "TRADE" ? (
            <>
              ...
              <span>{primaryActionLabel || (isKa ? "გაცვლის შეთავაზება" : "Propose Swap")}</span>
            </>
          ) : transactionType === "GIFT" ? (
            <>
              <Gift className="w-4 h-4" />
              <span>{primaryActionLabel || (isKa ? "მცენარის დაჯავშნა" : "Claim Plant")}</span>
            </>
          ) : (
            <>
              <CreditCard className="w-4 h-4" />
              <span>{primaryActionLabel || (isKa ? "უსაფრთხო ყიდვა (ბარათით / კურიერით)" : "Buy Safely with Buyer Protection")}</span>
            </>
          )}
        </Button>

        {/* ── SECONDARY CONTACTS (Inline 2 columns) ── */}
          ...
          <Button
            ...
            onClick={() => {
              if (!showFullPhone) {
                ...
                window.location.href = `tel:${safePhone.replace(/\s+/g, "")}`;
              }
            }}
        ...
        {/* ── Virtual Greenhouse Bridge (Subtle utility) ── */}
