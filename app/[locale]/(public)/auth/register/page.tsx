...
function RegisterPageContent() {
  ...
  const [promoCode, setPromoCode] = React.useState(refParam);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          full_name: fullName,
          referred_by_code: promoCode.trim() || undefined,
        },
      },
    });
    ...
    if (error) {
      ...
      if (data.session) {
        ...
        setSuccessMsg("რეგისტრაცია წარმატებით დასრულდა! გთხოვთ შეამოწმოთ თქვენი ელ-ფოსტა ვერიფიკაციისთვის.");
        setLoading(false);
      }
    }
  };
      ...
      <div className="rounded-3xl border border-border/80 bg-card p-6 sm:p-8 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            შემოუერთდით საქართველოს მცენარეთა საზოგადოებას
          </p>
        </div>
        ...
        {successMsg && (
          <div className="mb-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 p-4 text-xs text-emerald-700 dark:text-emerald-300 flex items-center gap-2">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            <label className="text-xs font-bold text-muted-foreground mb-1 block">
              პრომო / რეფერალ კოდი (არასავალდებულო)
            </label>
            <div className="relative">
...
export default function RegisterPage() {
  return (
    <React.Suspense fallback={<div className="container mx-auto px-4 py-16 text-center text-sm">იტვირთება...</div>}>
      <RegisterPageContent />
    </React.Suspense>
  );
}
