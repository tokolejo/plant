...
export default function ListingDetailPage({
  ...
  const [createOfferModalOpen, setCreateOfferModalOpen] = React.useState(false);
  ...
  // Handle Primary CTA (Escrow vs Swap vs Gift)
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            {listing.transactionType === "TRADE" && (
              <ListingTradeBox
                ...
                onProposeTrade={handlePrimaryAction}
                isKa={isKa}
              />
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 block lg:hidden border-t border-border/80 bg-background/95 backdrop-blur-md p-3 px-4 shadow-lg safe-area-bottom">
        <div className="flex items-center justify-between gap-3 max-w-md mx-auto">
          ...
          {/* Right: Primary Conversion Action */}
          <button
            type="button"
            onClick={handlePrimaryAction}
            className={`h-10 px-5 rounded-[12px] font-black text-xs text-white shadow-xs flex items-center justify-center gap-1.5 shrink-0 cursor-pointer active:scale-95 transition-all ${
              listing.transactionType === "TRADE" ? "bg-indigo-600" : "bg-primary"
            }`}
          >
            {listing.transactionType === "TRADE" 
              ? (isKa ? "გაცვლა" : "Swap") 
      ...
      {createOfferModalOpen && (
        <CreateOfferModal
          ...
          onOfferCreated={() => {
            ...
            router.push(`/dashboard/messages?listing=${listing.id}&seller=${listing.seller?.id || listing.user_id}`);
          }}
        />
      )}
    </div>
